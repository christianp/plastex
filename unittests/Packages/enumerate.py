...
def test_enumerate():
    ...
    assert enums[2].listType == 'a)'
    assert enums[2].term(items[0].position) == 'a)'
    assert enums[2].term(items[1].position) == 'b)'

    items = enums[3].getElementsByTagName('item')
    assert enums[3].listType == '<I>'
    assert enums[3].term(items[0].position) == '<I>'
    assert enums[3].term(items[11].position) == '<XII>'
